...
(defmacro vec2 [a b] `(vector-of :float ~a ~b))
...
(defmacro -make-vec-length []
  (cons `do
        (for [n [2 3 4]]
          `(defn ~(str-sym- "v" n "length")
             ~(str "Length of " n "-vector") [~'a]
             (numeric/sqrt (~(str-sym- "v" n "dot") ~'a ~'a))))))

(-make-vec-length)             

(defmacro -make-vec-generator [f is-fn]
  (cons `do
        (for [n [2 3 4]]
          `(defn ~(str-sym- "v" n f)
             ~(str "Generate " n " vector with entries generated by f")
             []
             (vector-of :float ~@(for [c (range n)] (if is-fn `(~f) f)))))))

(-make-vec-generator rand true)

(defn v2normalize [a] (v2muls a (/ (max eps (v2dot a a)))))
(defn v3normalize [a] (v3muls a (/ (max eps (v3dot a a)))))
(defn v4normalize [a] (v4muls a (/ (max eps (v4dot a a)))))
